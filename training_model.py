...
def getImagesAndLabels(path):       #method to get the images and label data
    ...
    imagePaths = [os.path.join("dataset/",f) for f in os.listdir(path)]      # Get all file path
...
assure_path_exists('lock_unlock_face_recognition.py')    # Save the model into trainer.yml
