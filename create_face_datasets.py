...
face_cascade = cv2.CascadeClassifier(r"haarcascade_frontalface_default.xml") # Detect object in video stream using Haarcascade Frontal Face
face_id = 1  # For each person,there will be one face id
count = 0    # Initialize sample face image
check_path("dataset/")
while(True):
    _,image_frame = vid_cam.read()       # Capture video frame _, is used to ignored first value because vid_cam.read() is returning 2 value
